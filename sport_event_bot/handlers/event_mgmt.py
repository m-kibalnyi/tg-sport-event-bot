...
async def show_info(update, context):
    ...
    if update.callback_query:
        await update.callback_query.edit_message_text(full_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
    else:
        msg = await update.message.reply_text(full_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML, disable_web_page_preview=True, message_thread_id=lists_thread_id)
        db.save_latest_bot_message(chat_id, msg.message_id, full_text)

def parse_cmd_arg(update, context) -> str:
    if context.args: return " ".join(context.args)
    return ""
