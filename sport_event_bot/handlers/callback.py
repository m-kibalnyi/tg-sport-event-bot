...
async def button(update, context):
    ...
    if data == "ADD":
        ...
        if event_id:
            await log_event(chat_id, event_id, f"User {query.from_user.first_name} joined event", context)
    elif data == "REMOVE":
        db.revoke_application_for_the_event(chat_id, user_id)
        db.set_event_extra1(chat_id, None)
        event_id = db.get_event_id_by_chat_id(chat_id)
        if event_id:
            await log_event(chat_id, event_id, f"User {query.from_user.first_name} left event", context)
    elif data == "THINK":
        ...
        event_id = db.get_event_id_by_chat_id(chat_id)
        if event_id:
            await log_event(chat_id, event_id, f"User {query.from_user.first_name} added guest player", context)
        ...
        event_id = db.get_event_id_by_chat_id(chat_id)
        if event_id:
            await log_event(chat_id, event_id, f"User {query.from_user.first_name} removed guest player", context)
    elif data == "REMOVE_ALL_LEGIONEERS":
        db.revoke_all_user_legioneers(chat_id, user_id)
        db.set_event_extra1(chat_id, None)
        event_id = db.get_event_id_by_chat_id(chat_id)
        if event_id:
            await log_event(chat_id, event_id, f"User {query.from_user.first_name} removed all guest players", context)
    elif data == "PAY":
...
async def handle_shuffle_callback(update, context, data):
    ...
    if extra1:
        try:
            num_teams = json.loads(extra1).get("num_teams", 2)
        except Exception:
            pass
    if data == "INC_TEAMS":
        num_teams += 1
    ...
    db.set_event_extra1(
        chat_id, json.dumps({"num_teams": num_teams, "teams": teams, "reserve": res_p}, ensure_ascii=False)
    )
