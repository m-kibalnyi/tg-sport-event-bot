...
def _build_content(event_title: str, entries: List[Tuple]) -> str:
    ...
    nodes.append({"tag": "br"})
    nodes.append({"tag": "p", "children": [f"Обновлено: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"]})
    return json.dumps(nodes, ensure_ascii=False)


def publish_payment_log_sync(event_title: str, entries: List[Tuple], existing_url: Optional[str]) -> str:
    ...
    if existing_url:
        ...
        try:
            result = _api_call("editPage", access_token=token, path=path, title=title, content=content)
            return result["url"]
        except Exception:
            pass  # Page may be gone — fall through to create new

    result = _api_call("createPage", access_token=token, title=title, content=content, author_name="Sport Event Bot")
    return result["url"]


async def publish_payment_log(event_title: str, entries: List[Tuple], existing_url: Optional[str]) -> str:
