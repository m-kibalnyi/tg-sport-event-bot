...
def create_event_full_text(chat_id: int, translate) -> str:
    ...
    try:
        ...
        logger.info(f"Event details: id={event_id}, limit={limit}, dt={dt_str}, loc={location}, pay={payment_url}, tph={telegraph_url}, blik={blik_phone}, web={web_url}")
        ...
        if dt:
            ...
            if dt > now:
                ...
                time_left_str = f"\n⏳ {translate('Time left')}: {days} {translate('days')} {translate('and')} {hours} {translate('hours')}"
            else:
        ...
        if web_url:
            u = web_url
            if 'payments.php' in u: u = u.replace('payments.php', 'event.php')
            elif 'event.php' not in u:
                 if not any(ext in u for ext in ['.php', '.html']):
                     u = u.rstrip('/') + '/event.php'
            
            if event_id:
                if 'localhost' in u: u = u.replace('https://localhost', 'http://localhost')
        ...
        if web_url:
            ...
            if 'localhost' in base_url: base_url = base_url.replace('https://localhost', 'http://localhost')
