...
def update_event_text(chat_id: int, text: str):
    conn = reconnect()
    _exec(conn, "UPDATE Events SET description = %s WHERE status = 'Open' AND chat_id = %s AND platform = %s;", (text, chat_id, PLATFORM))
    conn.close()
...
def set_players_limit(chat_id: int, limit: int):
    conn = reconnect()
    _exec(conn, "UPDATE Events SET players_limit = %s WHERE status = 'Open' AND chat_id = %s AND platform = %s;", (limit, chat_id, PLATFORM))
    conn.close()
...
def set_event_datetime(chat_id: int, dt_str: str):
    conn = reconnect()
    _exec(conn, "UPDATE Events SET datetime = %s WHERE status = 'Open' AND chat_id = %s AND platform = %s;", (dt_str, chat_id, PLATFORM))
    conn.close()
...
def get_event_location(chat_id: int) -> Optional[str]:
    conn = reconnect()
    cur = _exec(conn, "SELECT location FROM Events WHERE status = 'Open' AND chat_id = %s AND platform = %s LIMIT 1;", (chat_id, PLATFORM))
    row = cur.fetchone()
    conn.close()
    return row[0] if row else None
...
def set_event_extra1(chat_id: int, val: str):
    conn = reconnect()
    _exec(conn, "UPDATE Events SET extra1 = %s WHERE status = 'Open' AND chat_id = %s AND platform = %s;", (val, chat_id, PLATFORM))
    conn.close()
...
def fix_event(chat_id: int):
    conn = reconnect()
    _exec(conn, "UPDATE Events SET status = 'Closed' WHERE status = 'Open' AND chat_id = %s AND platform = %s;", (chat_id, PLATFORM))
    conn.close()
