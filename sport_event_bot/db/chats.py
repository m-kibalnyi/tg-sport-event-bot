...
def get_latest_bot_message_text(chat_id) -> str:
    ...
    cur = _exec(
        conn,
        "SELECT latest_bot_message_text FROM Chats WHERE chat_id = %s AND platform = %s LIMIT 1;",
        (chat_id, PLATFORM),
    )
    row = cur.fetchone()
    conn.close()
...
def save_latest_bot_message(chat_id, message_id, message_text):
    ...
    _exec(
        conn,
        "UPDATE Chats SET latest_bot_message_id = %s, latest_bot_message_text = %s WHERE chat_id = %s AND platform = %s;",
...
def set_log_thread_id(chat_id: int, thread_id: Optional[int]):
    ...
    _exec(
        conn,
        "UPDATE Chats SET log_thread_id = %s WHERE chat_id = %s AND platform = %s;",
        (thread_id, chat_id, PLATFORM),
    )
    conn.close()
...
def set_lists_thread_id(chat_id: int, thread_id: Optional[int]):
    ...
    _exec(
        conn,
        "UPDATE Chats SET lists_thread_id = %s WHERE chat_id = %s AND platform = %s;",
        (thread_id, chat_id, PLATFORM),
    )
    conn.close()
