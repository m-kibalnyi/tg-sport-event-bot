...
try:
    from sport_event_bot.db.base import _exec, reconnect
except (ImportError, ValueError):
    from .base import _exec, reconnect
...
def get_event_logs(event_id: int) -> List[Tuple[str, datetime.datetime]]:
    ...
    cur = _exec(
        conn,
        "SELECT message, operation_datetime FROM EventLogs WHERE event_id = %s ORDER BY operation_datetime DESC;",
        (event_id,),
    )
    rows = cur.fetchall()
    conn.close()
